...
async function getNextSessionNumber(userId: string): Promise<number> {
  const [workoutStats, planStats] = await Promise.all([
    prisma.workouts.aggregate({
      where: { userId },
      _max: { sessionNumber: true },
    }),
    prisma.plan_sessions.aggregate({
      where: { userId },
      _max: { sessionNumber: true },
    }),
  ]);

  const maxWorkout = workoutStats._max.sessionNumber ?? 0;
  const maxPlan = planStats._max.sessionNumber ?? 0;
  return Math.max(maxWorkout, maxPlan) + 1;
}
