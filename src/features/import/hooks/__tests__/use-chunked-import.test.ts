...
describe('useChunkedImport', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockBulkImportSessions.mockResolvedValue({ count: 20, skipped: 0, message: 'ok' });
  });
  ...
  it('starts idle', () => {
    ...
    expect(result.current.progress).toEqual({ imported: 0, skipped: 0, total: 0 });
    expect(result.current.importedKeys.size).toBe(0);
  });
  ...
  it('imports a single batch', async () => {
    ...
    const externalIds = ['ext-0', 'ext-1', 'ext-2', 'ext-3', 'ext-4'];

    let importResult: { imported: number; skipped: number; total: number } | undefined;
    await act(async () => {
      importResult = await result.current.start(sessions, externalIds);
    });
  ...
  it('splits into multiple batches of 20', async () => {
    mockBulkImportSessions
      .mockResolvedValueOnce({ count: 20, skipped: 0, message: 'ok' })
      .mockResolvedValueOnce({ count: 20, skipped: 0, message: 'ok' })
      .mockResolvedValueOnce({ count: 5, skipped: 0, message: 'ok' });

    const { result } = renderHook(() => useChunkedImport());

    const sessions = Array.from({ length: 45 }, (_, i) => makeSession(i));
    const externalIds = Array.from({ length: 45 }, (_, i) => `ext-${i}`);

    await act(async () => {
      await result.current.start(sessions, externalIds);
    });
    ...
    expect(result.current.status).toBe('done');
    expect(result.current.progress).toEqual({ imported: 45, skipped: 0, total: 45 });
  });
  ...
  it('stops on error and keeps partial progress', async () => {
    ...
    const externalIds = Array.from({ length: 40 }, (_, i) => `ext-${i}`);
    ...
    expect(result.current.progress).toEqual({ imported: 20, skipped: 0, total: 40 });
    expect(result.current.importedKeys).toEqual(new Set(Array.from({ length: 20 }, (_, i) => `ext-${i}`)));
  });

  it('cancels after current batch', async () => {
    let resolveSecondBatch: ((v: { count: number; skipped: number; message: string }) => void) | undefined;
    mockBulkImportSessions
      .mockResolvedValueOnce({ count: 20, skipped: 0, message: 'ok' })
    ...
    const externalIds = Array.from({ length: 60 }, (_, i) => `ext-${i}`);

    let startPromise: Promise<{ imported: number; skipped: number; total: number }>;
    await act(async () => {
      startPromise = result.current.start(sessions, externalIds);
    });
    ...
    await act(async () => {
      resolveSecondBatch!({ count: 20, skipped: 0, message: 'ok' });
      await startPromise!;
    });
